...
class DBManager {
    ...
    async getAgendas(){
        const agendaArray = [];
        const snapshot = await db.collection("agenda").listDocuments();
        for(let i=0; i<snapshot.length; i++){
            const snap = await db.collection("agenda").doc(snapshot[i].id).get();
    ...
    addAgenda(task, id){
        ...
        // console.log(data);
        db.collection("agenda").doc(id.toString()).set(data);
    }

    updateAgenda(task="", id, isCompleted=null, time=""){
        if(task != ""){
            db.collection("agenda").doc(id.toString()).update({task: task});
        }else{
            db.collection("agenda").doc(id.toString()).update({time: time, isCompleted: isCompleted});
        }
        
    }

    deleteAgenda(id){
        db.collection("agenda").doc(id.toString()).delete();
    }
