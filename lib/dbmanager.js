...
class DBManager {
    ...
    updateTotalToSapphire(week, year, obj) {
        db.collection("sapphire").doc("Sayantan").collection(year).doc(week).update(obj);
    }
    ...
    async getAgendas(date){
        const agendaArray = [];
        const snapshot = await db.collection("agenda").doc(date).collection("tasks").listDocuments();
        for(let i=0; i<snapshot.length; i++){
            const snap = await db.collection("agenda").doc(date).collection("tasks").doc(snapshot[i].id).get();
            agendaArray.push({id: snapshot[i].id, task : snap.data().task, time: snap.data().time, isCompleted: snap.data().isCompleted});
        }
        return agendaArray;
    }
    ...
    addAgenda(date, task, id){
        ...
        // console.log(data);
        db.collection("agenda").doc(date).collection("tasks").doc(id.toString()).set(data);
    }

    updateAgenda(date, task="", id, isCompleted=null, time=""){
        if(task != ""){
            db.collection("agenda").doc(date).collection("tasks").doc(id.toString()).update({task: task});
        }else{
            db.collection("agenda").doc(date).collection("tasks").doc(id.toString()).update({time: time, isCompleted: isCompleted});
        }
        
    }

    deleteAgenda(date,id){
        db.collection("agenda").doc(date).collection("tasks").doc(id.toString()).delete();
    }
    ...
    async initRoster() {
        ...
        if (snapshot.length == 0) {
            for (let i = 0; i < weekArray.length; i++) {
                await db.collection("roster").doc(weekArray[i]).set({ '11AM': "/-select-", '1PM': "/-select-", '3PM': "/-select-", '5PM': "/-select-", '7PM': "/-select-" });
            }
        }else{
